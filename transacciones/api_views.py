...
class AllCierresFilter(filters.BaseFilterBackend):
    ...
    def filter_queryset(self, request, queryset, view):
        ...
        if date_range:
            ...
            query = query & Q(fecha__range=[start_date, end_date])

        else:
            now = datetime.now()
            start_of_today = datetime(now.year, now.month, now.day)
            end_of_today = start_of_today + timedelta(days=1) - timedelta(seconds=1)
            query = query & Q(fecha__range=[start_of_today, end_of_today])

        return queryset.filter(query)


class ListarAllCierresAPIView(generics.ListAPIView):

    pagination_class = DataTableLongPagination
    serializer_class = se.CierresListSerializer
    permission_classes = [permissions.IsAuthenticated]
    filter_backends = [DataTableSearchFilter, AllCierresFilter]
    search_fields = ['id']

    def get_queryset(self):
        company = self.request.user.company_profile
        queryset = CierreCaja.objects.filter(compania=company)
        return queryset
